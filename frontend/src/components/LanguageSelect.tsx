...
const LanguageSelect: React.FC<Props> = ({ setFullQuery, query, language, setLanguage }) => {
    const handleOnChange = (lang: any) => {
        setFullQuery(`${query}&language=${lang.value}`);
        setLanguage(lang.value);
    };

    return (
        <div style={{display: 'inline-block', width: 200, marginRight: 50}}>
            <Select options={options} defaultValue={options[0]} onChange={lang => handleOnChange(lang)} />
        </div>
    );
};
